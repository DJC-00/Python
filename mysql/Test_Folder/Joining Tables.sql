-- Joining tables
-- JOIN
-- Find all the cliends (first and last name) billing amounts and charged date
...
SELECT clients.first_name, clients.last_name, SUM(billing.amount) AS total, MIN(billing.amount) AS Smallest, MAX(billing.amount) AS Largest
FROM clients
JOIN billing ON clients.id = billing.clients_id
GROUP BY clients.id;
